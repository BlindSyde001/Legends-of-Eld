...
/*void ARTSPlayerController::Tick(float DeltaSecond)
{
    Super::Tick(float DeltaSecond);
...
bool ARTSPlayerController::GetObjectsInSelectionFrame(TArray<FHitResult>& HitResults) const
{
    ...
    for (TActorIterator<AActor> ActorIt(GetWorld()); ActorIt; ++ActorIt)
    {
        ...
        URTSSelectableComponent* SelectableComponent = Actor->FindComponentByClass<URTSSelectableComponent>();
        URTSOwnerComponent* OwnerComponent = Actor->FindComponentByClass<URTSOwnerComponent>();
            
        if (SelectableComponent)
        {

            if (SelectableComponent->Selectable && OwnerComponent->IsSameTeamAsController(PlayerState->GetOwningController()))
            {
...
void ARTSPlayerController::PlayerTick(float DeltaTime)
{
    ...
    // Enforce camera bounds.
    if (!CameraBoundsVolume || CameraBoundsVolume->EncompassesPoint(Location) && IsInputKeyDown(EKeys::MiddleMouseButton))
    {
    ...
    if (IsValid(PlayerPawnCamera))
    {
        ...
        //CameraLocation.Z = FMath::Clamp(CameraLocation.Z, MinCameraDistance, MaxCameraDistance); SIEBARTH: erstmal ausgeklammert damit wir komplett Zoomen können 
