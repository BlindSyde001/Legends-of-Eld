...
void URTSSelectableComponent::SetMaterialDecal(uint8 team)
{
    if (team == 0)
    {
        DecalComponent->CreateDynamicMaterialInstance()->SetVectorParameterValue(TEXT("Color"), FLinearColor::Blue);
    }
    else
    {
        DecalComponent->CreateDynamicMaterialInstance()->SetVectorParameterValue(TEXT("Color"), FLinearColor::Red);
    }
}
